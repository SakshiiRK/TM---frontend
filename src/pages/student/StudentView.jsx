...
import { FaCalendarDay, FaCalendarAlt, FaClipboardCheck, FaInfoCircle, FaBook, FaCode, FaUserTie, FaMapMarkerAlt, FaClock } from 'react-icons/fa'; // Streamlined and relevant icons
...
const StudentView = () => {
  ...
  const fetchStudentTimetable = async (selectedDate) => {
    setLoading(true);
    setError('');
    setTimetableSlots([]); // Clear previous slots
    ...
    try {
      ...
      setError('Failed to fetch timetable data. Please check your network or try again.');
      setTimetableSlots([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (user.department && user.section && user.semester) {
        ...
        setError("User data (department, section, semester) not found. Cannot load timetable.");
    }
  }, [date, user.department, user.section, user.semester]); // Added user details to dependencies
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 text-gray-100 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center">
        ...
        <h2 className="text-4xl sm:text-5xl font-extrabold text-white mb-10 text-center animate-fade-in-down">
          <FaCalendarDay className="inline-block mr-4 text-pink-400 text-4xl sm:text-5xl animate-pop-in" />
          My Class Schedule
        </h2>
        ...
        <div className="bg-gray-800/70 rounded-xl shadow-md p-6 mb-8 text-gray-200 text-center border border-gray-700 animate-fade-in-up animation-delay-300 w-full max-w-2xl">
            ...
            <span className="font-semibold text-lg">Viewing timetable for: </span>
            <span className="text-white font-bold">{user.department}</span> - Semester <span className="text-white font-bold">{user.semester}</span>, Section <span className="text-white font-bold">{user.section}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full max-w-6xl">
          ...
          <div className="bg-gray-800/90 rounded-2xl shadow-xl p-6 border border-gray-700 backdrop-blur-sm transform transition-all duration-700 opacity-0 animate-fade-in-scale-up animation-delay-200">
            <h3 className="text-white text-2xl font-bold mb-6 flex items-center gap-3">
              <FaClipboardCheck className="text-purple-400 text-2xl" /> Schedule for {date.toLocaleDateString()}
            </h3>

            {loading ? (
              <div className="text-center py-8">
                <div className="inline-block animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-indigo-600"></div>
                <p className="mt-4 text-gray-300 text-lg">Loading your class schedule...</p>
              </div>
            ) : error ? (
              <div className="bg-red-900/40 text-red-300 p-6 rounded-lg border border-red-700 text-center">
                ...
                {(!user.department || !user.section || !user.semester) && (
                    <p className="mt-2 text-sm text-red-400">Please ensure your user profile has Department, Section, and Semester set.</p>
                )}
              </div>
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto custom-scrollbar"> {/* Added scrollbar and max-height */}
                {timetableSlots.map((slot, i) => (
                  <div key={i} className="bg-gray-700/50 rounded-lg p-4 border border-gray-600 transition-all duration-200 hover:bg-gray-700/70 transform hover:scale-[1.01] animate-stagger-fade-in" style={{ animationDelay: `${0.3 + i * 0.1}s` }}>
                    <h4 className="flex items-center text-teal-300 text-xl font-semibold mb-2">
                      <FaBook className="text-teal-300 text-2xl mr-3" />
                      {slot.courseName}
                    </h4>
                    ...
                    <p className="flex items-center text-gray-300 text-base mb-1">
                      <FaUserTie className="text-purple-300 text-lg mr-3" />
                      <strong className="font-medium text-purple-300 mr-2">Faculty:</strong> {slot.facultyName}
                    </p>
              ...
              <div className="text-center py-8 bg-gray-700/30 rounded-lg p-6 border border-gray-600">
                ...
                <p className="mt-2 text-gray-500 text-sm">Select another date or ensure your profile is complete.</p>
              </div>
            )}
          </div>
        </div>
